...
runs_data = parse_run(int(subject_id[0]), 1, EEG_channels, MEG_channels)
logger.info("logging...")
...
def change_run(attr, old, new):
    ...
    run = int(new.split(" ")[1])
    runs_data = parse_run(int(subject_id[0]), run, EEG_channels, MEG_channels)

    EEG_data = runs_data[0]
    MEG_data = runs_data[1]

    for idx, EEG_source in enumerate(EEG_sources[0]):
    ...
    for idx, MEG_source in enumerate(MEG_sources[0]):
